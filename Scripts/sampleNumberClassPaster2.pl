#!/usr/bin/perl
	...
	opendir(DIR, $dir1) or die $!;
        ...
        next unless ($file =~ m/\_transposed.out/);
...
system("paste $dir1/$file $dir2/classList.txt > $dir1/tempA");

system("sed -e 's/ /,/g' -e 's/\t/,/g' $dir1/tempA > $dir1/$partialName[0]'dataComplete.out'");
