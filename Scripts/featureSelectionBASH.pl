...
system("java -Xss1G -Xmx5G weka.attributeSelection.CfsSubsetEval -s \"weka.attributeSelection.BestFirst -D 1 -N 5\" -i $dir1/$file > $dir2/$partialName[0]'1.txt' ");
# Cfs Subset Evaluator + Genetic Search
} if ($Config{cfsSubsetEvaluatorGeneticSearch} eq "TRUE") {
system(" java -Xmx5G weka.attributeSelection.CfsSubsetEval -s \"weka.attributeSelection.GeneticSearch -Z 20 -G 20 -C 0.6 -M 0.033 -R 20 -S 1\" -i $dir1/$file > $dir2/$partialName[0]'2.txt' ");
}
# Cfs Subset Evaluator + Greedy Stepwise
if ($Config{cfsSubsetEvaluatorGreedyStepwise} eq "TRUE") {
system(" java -Xmx5G weka.attributeSelection.CfsSubsetEval -s \"weka.attributeSelection.GreedyStepwise -T -1.7976931348623157E308 -N -1\" -i $dir1/$file > $dir2/$partialName[0]'3.txt' ");
}
# Cfs Subset Evaluator + Linear Forward Selection
if ($Config{cfsSubsetEvaluatorLinearForwardSelection} eq "TRUE"){
system(" java -Xmx5G weka.attributeSelection.CfsSubsetEval -s \"weka.attributeSelection.LinearForwardSelection -D 0 -N 5 -I -K 50 -T 0\" -i $dir1/$file > $dir2/$partialName[0]'4.txt' ");
}
